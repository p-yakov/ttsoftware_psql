create table if not exists users_roles (id          int         generated always as identity --уникальный идентификатор таблицы
...
create unique index if not exists idx_unq_users_id             on users_roles (id);
create unique index if not exists idx_unq_users_roles_users_id on users_roles (users_id);
